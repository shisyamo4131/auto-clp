...
現在の範囲には、積荷・コンテナ入力、3D表示、手動移動・回転、境界・重なり・開口部・段積み可否・重量上限・軸別隙間の検証、端末内保存とJSON入出力を含みます。自動配置提案は後続マイルストーンです。搬出順を考慮した積み込み順、クラウド保存、アカウント、外部API、公開デプロイ、収益化は現在の範囲外です。
