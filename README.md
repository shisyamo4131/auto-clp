...
ローカルWebアプリの骨格、WebGL 2能力確認画面、版付き案件JSONのdomain・検証・取引的読込基盤、案件・積荷・コンテナ候補の入力編集UIを実装済みです。入力はブラウザのメモリ内だけに保持され、再読み込みすると失われます。現在表示される3D画面は技術確認用で、配置、制約判定、端末保存・JSON入出力UI、自動提案はまだ利用できません。
