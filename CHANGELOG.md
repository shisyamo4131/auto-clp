...
- 仕様1.6.0・ADR 0034で、Excel向け固定CSVテンプレートと積荷一括置換を承認した。手動追加とCSV作成は一つの定数による30件上限、段積みOK・天地無用OFFを共通既定とし、Schema `0.1.0` の積荷・配置上限1,000件と既存31〜1,000件の読込・編集・削除・書出し互換は維持する。CSVは固定見出し、厳格なUTF-8・値検証、決定的ID、件数付き確認を使い、確定時だけ積荷を置換して配置を全削除し、一回のUndo/Redo対象とする。現在は仕様・受入条件のみ確定済みで未実装・未検証であり、新規5点を未獲得として進捗を97%から92%へ補正した。
