...
- 仕様1.5.0・ADR 0032に従い、選択中コンテナ内の保存済み積荷だけをBigInt・有理数で重量付き集計し、コンテナ幾何中心を赤、現在の積荷合成重心を黄の操作不可ドットとして3D内に表示する機能を実装した。画面投影中心一致は位置をずらさない同心表示、近接は各投影中心を保つ大きさ・輪郭・重ね順、画面外はclampやcamera自動変更をしない非数値statusとし、コンテナなし・空・利用可能・計算不能を区別して古い派生表示を残さない。DPR、camera、drag、履歴、読込成功・失敗、破損参照の読取専用回復表示、狭幅を全単体29ファイル975件・全ブラウザ97件で回帰し、型・lint・build・文書検査と独立コードレビューに合格した。使用上の重要事項は内容版1.2.0へ更新した。数値偏差、許容範囲、合否、安全保証、コンテナ自重、drag preview追従、永続化は対象外で、Schema `0.1.0` は変更しない。人間による差分視認性確認を残すため重量バランス可視化は4/5、現在進捗は97%とする。
