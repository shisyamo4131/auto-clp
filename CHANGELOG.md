...
- 仕様1.5.0・ADR 0032で、選択中コンテナ内の保存済み積荷だけを重量付き集計し、コンテナ幾何中心を赤、現在の積荷合成重心を黄の操作不可ドットとして3D内に表示する方針を承認した。中心一致は位置をずらさない同心表示、近接は各投影中心を保つ大きさ・輪郭・重ね順、画面外はclampやcamera自動変更をしない非数値statusとし、空・計算不能・コンテナなしを区別して古い派生表示を残さない。数値偏差、許容範囲、合否、安全保証、コンテナ自重、drag preview、永続化は対象外とし、Schema `0.1.0` は変更しない。機能は計画済み・未実装であり、独立5ポイントのマイルストーン追加に伴って現在進捗を98%から93%へ再配分した。
