...
- Implemented rotation-toolbar refinement: X/Z回転はUndo/Redo・拡大縮小と同じviewport固定toolbarへ常設する。一本の軸線へ矢印が回り込む同一SVGをXだけ90度回して区別し、未選択・天地無用・busy・許可向きなしではfocus可能な理由付き `aria-disabled` とする。使用可は3Dの青緑線および積荷色と混同しない明るい紫色、使用不可は暗い低彩度の背景・枠・前景で区別し、回転前後でbutton位置は変えない。
