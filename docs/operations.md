...
- Implemented: Gitリポジトリ、ガバナンス、仕様、ロードマップ、ADR、案件JSON Schema `0.1.0`、完全なreadonly案件型、構造・意味検証、検証済みJSON書出し、派生計算成功後だけ置換する取引的読込基盤、ローカルWebアプリ骨格、WebGL 2能力確認、能力確認用Three.js描画、型・lint・単体・ブラウザ・ビルド検証。
- Planned: 案件入出力UIと端末保存、積荷・候補入力、3D手動配置、矩形開口・許可回転・幾何支持・総耐荷重の制約再計算、自動提案。
- Unavailable: UIからの実案件入力・配置・判定・保存・JSON入出力、デプロイ、クラウド保存、外部API、実運用サポート。
...
- 現在の成果物は本リポジトリ内の文書、設定、最小アプリ骨格である。
- 案件データの機械可読な設計契約は `schemas/project-0.1.0.schema.json`、意味契約は `docs/data-model.md` である。構造・意味検証、検証済み書出し、取引的読込の純粋な基盤は実装済みだが、File・端末保存・UIアダプターは未実装である。
- アプリのビルド出力は `dist/` であり、Git管理対象外とする。
- 現在の技術確認ビルドはJavaScript chunkがViteの500 kB推奨値を超える警告を出す。機能ゲートではないが、利用者向け機能の接続時に分割と初期読込性能を再評価する。
