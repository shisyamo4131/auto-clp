...
- Planned: Schema・意味検証からの安全なWorker境界、非永続preview、取消・stale、cutoff/no-complete-plan表示、確認付き一括適用とUndo。
...
自動提案の純粋domain探索だけが実装済みで、利用者向けWorker、preview、取消、適用は未実装である。現在は自動提案を利用可能として案内してはならない。後続実装でも探索開始だけでProjectを変更せず、preview、取消、cutoff、完全案なし、失敗、stale、積荷なし、候補なしでは現在案件を保持する。適用時だけ全配置を確認付きで一括置換し、一回のUndoで戻せるようにする。探索上限到達と完全案なしを実積載不能または安全性の証明として案内してはならない。より優先される候補がcutoffの時は、後続候補の完全案を目的関数上の最良として案内してはならない。
