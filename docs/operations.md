...
- Planned: ADR 0004に基づく、一候補へ全積荷を配置する非永続preview、決定的attempt上限、専用Worker、取消、cutoff/no-complete-plan表示、確認付き一括適用とUndo。
...
自動提案は未実装である。実装後も探索開始だけでProjectを変更せず、preview、取消、cutoff、完全案なし、失敗、stale、積荷なし、候補なしでは現在案件を保持する。適用時だけ全配置を確認付きで一括置換し、一回のUndoで戻せるようにする。探索上限到達と完全案なしを実積載不能または安全性の証明として案内してはならない。より優先される候補がcutoffの時は、後続候補の完全案を目的関数上の最良として案内してはならない。
