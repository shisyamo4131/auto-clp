...
CLP全体で未配置の積荷だけを初回は先頭許可向き・Z=0の決定的な暖色gridへ派生し、他候補へ配置済みなら重複表示しない。grid間隔は各積荷の許可向き全体から得る最大X/Y footprintで固定し、一つを回転しても他の未配置積荷を動かさない。利用者が積荷全体を荷室外へdropした後はcargo IDごとの位置と向きをUI sessionだけに保持し、Zは0 mmへ正規化する。外側移動と回転はProject、物理判定、CLP履歴、保存へ含めない。fine-pointer dragは量子化後のno-opを先に除き、X/Y footprintを完全包含・正面積partial・面積0 outsideへ分類する。完全包含またはpartialでは、支持可能面がなければ床、あれば最も高い支持可能上面へZをsnapし、単一面が底面を収容できる場合だけX/Yをその面内へ制限する。同じ最小Zの固定積荷側面とコンテナ内壁4面には50 mm以内でX/Y fitし、既定ONのsession-only `adjust` buttonでこのfitだけを切り替える。未配置と配置済みの双方でsnap後位置を一回の配置追加・更新として保存し、partial・条件未確認・不適合も修正途中として再判定する。outsideは未配置ならZ=0のsession poseだけ、配置済みなら一回の `placement.delete` と同じsession poseにする。Undoは元配置、Redoは同じ外側床面poseを表示する。
