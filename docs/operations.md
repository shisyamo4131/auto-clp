...
- Implemented: Gitリポジトリ、ガバナンス、仕様、ロードマップ、ADR、案件JSON Schema `0.1.0`、完全なreadonly案件型、構造・意味検証、検証済みJSON書出し、派生計算成功後だけ置換する取引的読込基盤、案件・隙間・積荷・候補の取引的な入力編集UI、ローカルWebアプリ骨格、WebGL 2能力確認、能力確認用Three.js描画、型・lint・単体・ブラウザ・ビルド検証。
...
配置座標はADR 0010のコンテナ局所右手座標を使い、`positionMm` は向き適用後の積荷直方体の最小角とする。正規値は整数mmを維持し、描画用中心、scene縮尺、camera、選択状態を案件へ保存しない。Project→scene投影と配置操作は未実装である。
