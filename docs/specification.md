...
- drag previewは床、単独支持、支持条件未確認、不適合、荷室外を色と固定高の操作statusで区別し、接触候補を強調する。drop後の区分と理由は同じ整数mm位置を物理判定へ渡して決める。不適合も修正途中として保存可能で、強制rollbackしない。一回のdropは一回の配置履歴とする。
