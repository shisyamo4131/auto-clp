...
- fine pointerで未配置積荷をdragし、向き適用後の直方体全体がコンテナの生の内部範囲へ入った場合だけ一回の配置追加として確定する。積荷全体が荷室外にあるdropはその作業位置を保持する。一部だけ荷室床面へ正面積で重なるdrop、取消、競合、no-opは案件と履歴を変更せず直前の作業位置を保持する。隙間、重なり、開口、支持、耐荷重は配置確定後の独立判定へ委ね、座標フォームによる不適合配置の保存・微調整経路は維持する。
- fine pointerで配置済み積荷を床面方向へdragした場合は、scene差分を整数mmのX/Yへ量子化した後に判定する。X/Yが変わらないno-opは案件と履歴を変更しない。向き適用後のX/Y占有範囲が生の荷室床面 `[0, L] × [0, W]` と正面積で重なる場合は配置を保持し、一部はみ出しも不適合な修正途中配置として一回の `placement.drag-xy` 履歴へ保存する。重なり面積が0、すなわち面・辺だけの接触を含め完全に外れた場合は、そのdrop位置と向きを非永続作業状態へ記録して既存配置を削除し、一回の `placement.delete` 履歴とする。Undoは元の位置と向きを復元し、Redoは同じ外側作業位置へ戻す。このgesture境界はZを使わず、床突き抜け、天井超過、その他のZ不適合は配置を保持して独立判定する。
- 3D viewport上のwheel入力はページscrollへ渡し、cameraを拡大・縮小しない。cameraの拡大・縮小は明示的な `＋` / `－` buttonだけで行い、「荷室全体を表示」を維持する。
- 3Dへ投影中の積荷は、viewport近傍のcompactな一覧からcargo IDで選択・highlightできる。選択だけではcameraを自動移動しない。
...
- 回転はX/Zを区別するicon-only buttonとし、可視の説明文を置かず、accessible nameとtitleで軸と無効理由を提供する。
...
- 案件履歴のUndo/Redoはviewport内で `＋` / `－` と同じ外観のtoolbarへ一組だけ置く。同一候補のProject更新ではcamera位置と注視点を保持し、Undo/Redo実行時のpage scroll位置も保持する。WebGL 2非対応時は同じ履歴操作をfallback領域で利用できる。
...
- 案件設定、積荷、候補コンテナ、配置の追加・更新・削除と、3D上の一回の床面方向dragによる配置更新または配置削除を、成功して案件を変更した単位ごとに最大100件まで取り消し・やり直しできる。
...
自動提案の目的関数、決定性、探索上限、打切り、適用境界はADR 0004、正規単位と値域、Phase 1の開口部モデル、積荷別許可向き、支持と荷重の扱いはADR 0005〜0008、軸別固定隙間の意味はADR 0011、物理制約の独立診断と集約はADR 0012、床突き抜けの専用診断はADR 0014、3D作業面・軸別回転・compact操作はADR 0017で確定した。匿名の合成受入契約は `acceptance.md` を正とする。完全な搬入経路、方向ラベル、部分支持、積荷別上載荷重、重心、軸重、床荷重、荷崩れ、固縛、動荷重が代表ケースで必要になった場合は、後継ADRで範囲を拡張する。
