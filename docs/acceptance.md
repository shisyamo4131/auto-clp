...
10. コンテナ0件ではviewport中央に「コンテナを登録してください」と `登録` buttonを表示し、既存のコンテナ追加dialogを開く。Drawerではテンプレートインポートだけを理由付きで無効にし、テンプレートダウンロードとJSON読込は利用可能なままとする。積荷だけのJSON読込後も同じ案内を表示し、コンテナ登録後に積荷を3D表示する。
...
1. 匿名積荷20〜30件を一括登録し、複数の荷室外積荷を手動で遠方へ移動・回転した後、Undo左の `arrow-collapse-all` icon buttonを実行する。未配置積荷は現在向きを保持し、選択中コンテナの負X側へ100 mm以上の間隔で重ならず再整列する。寄せ前後でcamera位置、注視点、視点方向、縮尺は変わらず、Project、配置、履歴、端末保存、JSONも変わらない。同じ状態で再実行しても結果が変わらない。
2. 積荷selectorは `積荷名 — 長さ×幅×高さ mm／重量 kg` を表示し、検索結果件数を置かない。右側dotは未選択を灰、未配置を黄、現在コンテナ配置済みを青、別コンテナ配置済みを緑とし、読み上げ名とtitleでも区別できる。
3. 重心凡例の下で、選択中コンテナの総重量／耐荷重が正規g値と一致し、複数コンテナの配置を横断した積込済数／全積荷数が一意な積荷数と一致する。小数kgは最大3桁で末尾0を付けず、表示は耐荷重の合否理由を置き換えない。
4. viewer上のwheelでcameraが拡大・縮小し、ページ位置と履歴は変わらない。viewer外のwheelはページscrollを維持し、旧 `＋` / `－` buttonは表示しない。
...
## AC-12 Grounded Staging, Inner-Wall Fit, and Snap Toggle

このケースは仕様1.12.0・ADR 0042に従い、AC-11の荷室外ZとX/Y fit距離を更新する。ProjectとSchema `0.1.0`は変更しない。

1. 積荷上面にある積荷を完全に荷室外へdragすると、previewと確定session位置のZは0 mmになる。配置済みなら一回の `placement.delete` として確定し、Undoは元の積層配置、Redoは同じ荷室外床面位置を復元する。
2. 床または支持上面のZ決定後、同じ最小Zの固定積荷側面とコンテナ内壁4面をX/Y fit候補にする。距離50 mmでは隙間0 mmへfitし、51 mmではfitしない。候補は移動量、X軸、内壁、積荷ID・面方向の順で決定し、正体積重複する候補は採用しない。
3. viewport toolbarのMaterial Design Icons `adjust` 相当buttonは起動時ONで、押下ごとに積荷側面・内壁へのX/Y fitだけをOFF / ONできる。OFFでも床・支持上面へのZ snapと単一支持面内clampは有効である。
4. 切替はUI sessionだけに保持し、Project、JSON、端末保存、Undo/Redo履歴へ含めない。ページ再読込または新しい実行セッションでは既定ONへ戻る。
5. 物理判定buttonは仕様1.13.1でMaterial Design Icons `information-variant` 相当iconへ変更し、状態別の色、accessible name、title、件数、dialogを維持する。
6. viewport上段toolbarの全buttonは、可視iconをbuttonの上下左右中央へ配置する。
...
1. コンテナ内壁＋内壁、内壁＋積荷、異なる積荷＋積荷について、X軸最大1面とY軸最大1面の最終位置を一括検証し、安全な場合は対象積荷輪郭または内壁矩形枠を青緑で同時強調して「2面にフィット」と表示する。合成位置だけが立体重複する場合は安全な単一軸へfallbackする。
...
3. 支持と遮蔽の先行関係を合成しても循環しない場合、同じProjectは同じ順序を返す。同時候補は最小X降順、最小Z昇順、最小Y昇順、積荷ID昇順になる。
4. 循環、参照不整合、正体積重複、床より上の支持接触不明では、関係積荷と固定理由codeを返し、不完全な残り順序またはPDFを成功扱いしない。
5. 現行物理判定にその他の不適合がある場合、帳票へ状態と理由を載せる。順序または帳票を適合、安全、完全な経路と表現しない。
6. 一覧は順序番号、名称、ID、現在向き適用後寸法、kg重量を含み、現在視点・正面・背面・左面・右面の全画像で同じ番号が該当積荷に付く。
7. 正面は負Xから正X、背面は正Xから負X、左右は正面から内側を見る利用者基準である。固定4面は通常camera変更で向きが変わらず、現在視点だけが生成開始時cameraを反映する。
8. 1件、20件、30件で日本語textと全積荷行を含む複数ページ対応PDFをbrowser内で生成し、`.pdf` として取得できる。CLPデータまたは画像を外部へ送信しない。
9. PDFにはCLP／コンテナ名、生成日時、限定モデル、未評価項目、非保証を表示し、実在顧客名、個人情報、秘密情報をfixtureへ使わない。
10. 生成失敗、WebGL障害、container未選択または配置0件では不完全fileをdownloadせず、再試行可能な理由を示す。
...
- Phase 5自動証拠: `tests/browser/loading-report.spec.ts` が1・20・30件、単一支持積層、複数支持について一覧全件、番号付き5視点、PDF全行・5画像、支持先行、未確認表示を検証し、既存のdomain循環検出とbrowser提案不能を含む全ブラウザ123件に合格した。人間のプロジェクト評価者によるPDFの改ページ・フォント等の目視確認と合わせ、積込順提案・PDF帳票Phase 5を完了した。
- 2026-09-17の後続人間確認: プロジェクト評価者は、公開Chromeで観察済みの重心表示の視認性・理解度と、30件時の荷室外積荷の寄せ、4状態dot、wheel zoom、Ctrl panを受入可能と判定した。これは正式fixtureを用いた実務利用者受入または安全性評価ではない。
...
- AC-14 Phase 1〜5: `src/domain/loading-sequence.test.ts` が直線搬入帯、Y/Z非交差・境界接触、単一・部分・複数・支持不可接触、参照・幾何・正体積重複・支持接触不明、決定的tie-break、明示graph循環、入力順不変、別コンテナ除外、30件を検証する。`src/application/loading-report.test.ts` が非永続snapshot、現在向き寸法・重量・先行理由・物理判定を検証する。`src/scene/loading-report-images.test.ts` と `tests/browser/loading-report.spec.ts` が固定4面の座標向きとframing、荷室外積荷の除外、現在視点cameraの捕捉、固定面のcamera非依存、全配置積荷の一覧対応番号、5枚の完全set、canvas出力失敗時の中止とretry、選択コンテナ切替、提案不能、focus、305 / 320 / 375 pxを検証する。さらに `src/platform/loading-report-font.test.ts`、`src/persistence/loading-report-file.test.ts` とbrowser回帰が、静的Noto Sans JPの字形routing、範囲外文字の非置換、固定名・MIME・失敗code、PDF signature、3ページ以上、抽出可能な日本語、名称・ID・現在向き寸法・重量・非保証注記、5画像、フォント失敗時のdownload中止とretryを検証する。Phase 5は1・20・30件、単一支持積層、複数支持の一覧全件・5視点label・PDF全行・5画像を追加回帰し、既存の循環検出と提案不能回帰を統合する。匿名4件sampleのPoppler描画と人間によるPDF目視確認も完了した。
...
- 仕様0.18.0由来の向き回帰は、積荷editorに天地無用以外の向きcheckboxがないこと、旧1向き・横倒し部分集合を2向き・6向きへ正規化すること、天地無用でもZ軸床面回転が利用できること、天地無用OFFでX/Zとも利用できること、使用可の枠が他の利用可能toolbar buttonと一致し使用不可の枠・iconが低彩度であることを確認する。
