...
- AC-14 Phase 1〜3: `src/domain/loading-sequence.test.ts` が直線搬入帯、Y/Z非交差・境界接触、単一・部分・複数・支持不可接触、参照・幾何・正体積重複・支持接触不明、決定的tie-break、明示graph循環、入力順不変、別コンテナ除外、30件を検証する。`src/application/loading-report.test.ts` が非永続snapshot、現在向き寸法・重量・先行理由・物理判定を検証する。`src/scene/loading-report-images.test.ts` と `tests/browser/loading-report.spec.ts` が固定4面の座標向きとframing、荷室外積荷の除外、現在視点cameraの捕捉、固定面のcamera非依存、全配置積荷の一覧対応番号、5枚の完全set、canvas出力失敗時の中止とretry、選択コンテナ切替、提案不能、focus、305 / 320 / 375 px、未実装PDF入口の明示を検証する。画像のローカル人間確認、日本語PDFとPDF取得はPhase 4・5で追加する。
