...
1. 匿名積荷20〜30件を一括登録し、複数の荷室外積荷を手動で遠方へ移動・回転した後、Undo左の磁石buttonを実行する。未配置積荷は現在向きを保持し、選択中コンテナの負X側へ100 mm以上の間隔で重ならず再整列し、コンテナと未配置積荷全体が見える。Project、配置、履歴、端末保存、JSONは変わらず、同じ状態で再実行しても結果が変わらない。
