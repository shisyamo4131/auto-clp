...
PDF出力では、画面上の成功表示と固定ファイル名を確認した。Computer Useのdownload eventは捕捉できなかったため、この記録だけではダウンロード済みbinaryの再読込、文字抽出、ページ数、画像埋込みを再検証したとは扱わない。これらはPhase 4の自動証拠とは区別する。
...
- ダウンロード済みPDF binaryを開いて行う、日本語、改ページ、画像、番号対応の人間確認。
...
- 検証用JSONと生成scriptはリポジトリのignored `tmp/public-acceptance/` に置き、製品buildまたは保存形式へ含めていない。
