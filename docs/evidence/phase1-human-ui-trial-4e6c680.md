...
### HUT-01 Floor-cascade Final Re-trial

- Date: 2026-08-30
- Evaluator: 同じ人間のプロジェクト評価者
- Environment: ローカルin-app Browser、`http://127.0.0.1:4174/`、匿名合成データ「HUT-01 床貫通診断 再試験」
- Build: specification `0.18.1`、Schema `0.1.0`、commit `a2556921279a561ea01bd16121fd9af164068753`
- Result: guided human re-trial pass

正式な自動回帰と同じ派生fixtureとして、支持可能なAと上段Bを各 `1000×800×600 mm`、100 kg、`WLH`、X=200、Y=100とし、AをZ=-1、BをZ=600へ配置した。候補は内部 `4000×2400×2400 mm`、開口 `2200×2200 mm`、耐荷重1000 kg、隙間は全軸0である。評価者は、初期状態でAの床貫通1件だけが表示され、Bの支持不足、pair隙間、構造・安定性未確認が連鎖表示されないことを確認した。

AをZ=0へ修正すると床貫通が消え、A上面Z=600とB下面Z=600の単独支持が成立して、Bの構造・安定性未確認だけが表示された。UndoでAのZ=-1と床貫通1件だけの状態、RedoでZ=0と構造・安定性未確認へ正確に往復した。5項目すべてを期待どおりと評価した。

評価者は、修正前のA占有範囲がZ=-1〜599、B下面がZ=600で1 mm浮いている点を確認した。この隙間は、Aだけを本来の床位置Z=0へ直せば支持が成立する根本原因分離用の意図的条件であり、診断上のZ=0正規化は描画または保存座標を動かさない。各配置は独立した絶対最小角座標で、Aを動かしてもBは自動追従しない。上段もまとめて動かす操作は未実装であり、評価者は現行挙動と説明を了承した。
