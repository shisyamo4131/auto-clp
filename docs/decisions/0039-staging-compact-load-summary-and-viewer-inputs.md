...
- Status: Accepted
- Date: 2026-09-16
- Related specification: `../specification.md` 1.10.0
...
- Undoの左にaccessibleな磁石icon buttonを置く。実行時は全未配置積荷の現在向きを維持し、選択中コンテナの負X側へ100 mm間隔の決定的gridで再整列する。手動の荷室外位置は破棄し、コンテナと未配置積荷全体を収めるcameraへ合わせる。
...
- 仕様1.10.0へ更新し、Schema `0.1.0`、CSV、JSON、IndexedDB、既存Projectの意味は変更しない。データ移行は不要である。

## Verification

- 純粋scene試験で、遠方overrideを近接gridへ戻し、向き、非重複、決定性、Project非変異を確認する。
- browser試験で、4状態dotと色以外の名称、重量書式、総重量／耐荷重、複数コンテナ横断の積込済数、件数撤去、磁石buttonの非履歴、wheel zoomとpage位置、`＋` / `－` 撤去、Ctrl cursorと積荷上pan優先、狭幅、操作方法を確認する。
