...
- Three.js表示は `1 mm = 0.001 scene unit` を共通縮尺とし、Z-upのdomain座標を明示的なadapterで変換する。sceneの浮動小数値を正規案件へ逆変換しない。
...
- Tests: 6向き、奇数・偶数寸法、床・境界等値と±1mm、開口中央、scene変換、非変異を検証する。具体例として、内幅1,000mm・開口幅999mmの開口Y範囲は0.5〜999.5mm（2倍整数では1〜1,999）、積荷101×203×305mmは全6向きで負の最小角を含め `max = min + orientedDimension`、床 `z = 0`、各境界の等値と±1mmを確認する。

## Compatibility and Migration

検証済みserializerは存在するが、座標値を生成する配置UIまたはapplication command、利用者向けJSON入出力UI、端末保存、リリース済み保存データはまだ存在しない。このため、未定義だったSchema `0.1.0` の意味を最初の座標値生成機能の実装前に確定し、Schema版は据え置く。
