...
| 積込順提案・PDF帳票 | 12 | 8 | In progress | [機能ロードマップ](loading-sequence-pdf-report.md) Phase 0〜3として仕様1.14.0、ADR 0044、AC-14、`loading-sequence-v1` の直線搬入帯・支持先行・決定的graph・固定失敗理由、非永続の帳票snapshot、提案内容を確認するdialog、同一snapshotから作る現在視点・正面・背面・左面・右面の番号付き画像を実装。PDF生成とローカル受入は未着手 |
...
1. [積込順提案・PDF帳票ロードマップ](loading-sequence-pdf-report.md) Phase 4として、日本語の一覧と番号付き5視点画像を含むPDFをブラウザ内で生成し、外部送信なしで取得できるようにする。
...
| 積込順提案・PDF帳票 | [仕様1.14.0](../specification.md)、[ADR 0044](../decisions/0044-limited-loading-sequence-and-pdf-report.md)、[機能ロードマップ](loading-sequence-pdf-report.md)、[AC-14](../acceptance.md#ac-14-limited-loading-sequence-and-pdf-report) | Phase 0〜3完了。直線搬入帯、支持先行、決定的graph、固定失敗理由、非永続帳票snapshot、提案内容を示すdialogを実装。同一snapshotから、出力開始時cameraの現在視点とcamera非依存の正面・背面・左面・右面を専用render targetで生成し、全配置積荷へ一覧対応番号を表示する。PDF生成は未実装 | 全単体35ファイル1,062件、browser 115件、型、lint、build、データ契約、ガバナンス、プロジェクト検査に合格。固定視点の向き・framing、荷室外積荷の除外、番号対応、camera非依存、不完全画像setの拒否とretryを回帰。画像のローカル人間確認とPDFは後続Phaseで追加する |
