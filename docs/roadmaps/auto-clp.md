...
| コンテナ・配置の自動提案 | 15 | 9 | In progress | Worker基盤に加え、React非依存session、Project参照・generation stale、取消・retry race、source相関、固定copy、25件page viewを実装・単体検証。App・React未接続で、実Worker、preview DOM、適用、Undo、実時間性能は未実装 |
...
3. 自動提案sessionをReact hook/panelとAppへ接続し、busy・generation開始gate、実Worker取消・retry、非永続preview DOMを実装する。
...
| コンテナ・配置の自動提案 | [ADR 0004](../decisions/0004-optimization-objective.md)、[合成受入AP-01〜08](../acceptance.md#automatic-proposal-synthetic-cases) | 純粋探索・Worker基盤に加え、React非依存session/view、Project/generation stale、取消・retry、source相関、固定copy、25件pageを実装。App・利用者向け経路は未実装 | domain31件、Worker80件、session27件・view37件、全単体803件、型・lint・build、独立レビュー合格。React/実Workerブラウザ、適用、Undo、実時間AP-08は未実施 |
