...
| コンテナ・配置の自動提案 | 15 | 8 | In progress | 純粋探索に加え、正本Schema・意味検証、private brand、one-shot module Worker、厳格な応答guard、固定code client、即時terminate取消・遅延応答maskを実装・単体検証。App未接続で、Project/generation stale、preview、適用、Undo、実Worker性能は未実装 |
...
3. 自動提案WorkerをAppへ接続し、Project参照・interaction generationに基づくstale破棄、開始gate、取消・retry、非永続previewを実装する。
...
| コンテナ・配置の自動提案 | [ADR 0004](../decisions/0004-optimization-objective.md)、[合成受入AP-01〜08](../acceptance.md#automatic-proposal-synthetic-cases) | 純粋探索に加え、Schema・意味検証付きone-shot Worker、厳格なprotocol guard、固定code client、取消・遅延応答maskを実装。App・利用者向け経路は未実装 | domain専用31件、Worker protocol49件・engine16件・client15件、全単体739件、型・lint・build、独立レビュー合格。実Workerブラウザ、stale、preview、適用、Undo、実時間AP-08は未実施 |
