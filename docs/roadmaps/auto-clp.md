...
| --- | ---: | ---: | --- | --- |
| 基盤・データ契約 | 10 | 9 | In progress | ガバナンス、仕様、主要制約ADR、版付きJSON Schema、意味契約、最小アプリ骨格、WebGL 2能力ゲート、向き適用関数と検証を作成。案件全体の型・意味検証が残る |
...
1. JSON Schemaに対応する完全なdomain型、構文・意味検証、トランザクション読込を実装する。
2. 直方体表示に案件データを接続し、入力と手動配置の最小経路を作る。
...
| --- | --- | --- | --- |
| 基盤・データ契約 | [仕様](../specification.md)、[ADR索引](../decisions/README.md)、[データ契約](../data-model.md)、[JSON Schema](../../schemas/project-0.1.0.schema.json) | JSON Schema `0.1.0`、TypeScript/React/Three.js/Vite骨格、WebGL 2能力ゲート、向き適用関数 | データ契約・型・lint・単体・ブラウザ・ビルド・ガバナンス検証。完全な案件型と意味検証が残る |
