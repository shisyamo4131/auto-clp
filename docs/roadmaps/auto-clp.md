...
1. 仕様0.18.0の人間再試用として、端末保存済みfixtureを再読込し、全積荷でZ軸床面回転可、操作荷B（小）の天地無用でX不可、操作荷D（大）の天地無用OFFでX/Z可、積荷editorに個別向きcheckboxなし、zoomと同じ強調枠、2回連続回転時のbutton位置不変をChromeで確認する。
