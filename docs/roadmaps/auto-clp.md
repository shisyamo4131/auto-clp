...
| 3D表示と手動配置 | 25 | 6 | In progress | 座標契約、候補選択、Project→scene投影、コンテナ内部・中央開口・登録済み配置の描画、投影範囲適応cameraを実装。積荷の選択・移動・回転、視点操作、取り消しは未実装 |
...
| 3D表示と手動配置 | [ADR 0001](../decisions/0001-local-first-web-architecture.md)、[ADR 0002](../decisions/0002-cuboid-model.md)、[ADR 0010](../decisions/0010-container-coordinate-and-placement-anchor.md) | 座標契約、純粋な配置範囲・scene adapter、非永続の候補選択、コンテナ内部・中央開口・登録済み配置の描画、全投影範囲camera | 型・lint・単体162件・ブラウザ15件・ビルド・実UI確認・独立コードレビュー |
