...
3. 仕様1.2.1・ADR 0027の3項とShift付き左drag・右drag平行移動は同じ人間の評価者が期待どおりと確認済みである。仕様1.2.2・ADR 0028のDrawer内 `操作方法` dialog、compact寸法矢印、`現在の座標` copyを差分中心で確認し、Schema 0.1.0と進捗98%は実務利用者受入完了まで維持する。
