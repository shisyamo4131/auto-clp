...
1. 人間再試用で合格した仕様0.15.0の支持面snapに続き、仕様0.16.0のdrag集中表示を同じChromeデータで確認する。drag対象は状態色、他積荷はほぼ透明な中立面・灰色点線、支持候補は緑または黄の点線となり、drop・取消後に通常表示へ戻ることを観察する。
