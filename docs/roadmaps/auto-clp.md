...
1. [案内付き人間評価](../evidence/phase1-human-ui-trial-4e6c680.md)で必要性が確認された、明示的な `＋` / `－` と荷室基準の視点復元、荷室外の非永続仮置き場、選択積荷近傍の許可向き90度回転、scene近傍の座標・履歴操作、時間的に識別できる通知、保存Navigation Drawerを、既存commandと安全境界を保つUI改善として設計・実装・検証する。
