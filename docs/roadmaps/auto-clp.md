...
| コンテナ・配置の自動提案 | 15 | 6 | In progress | 設計に加え、Schema・意味検証済みProject用の純粋な決定的DFS、候補順位、向き・候補点列挙、10,000/1,000,000 attempt境界、cutoff/no-complete-plan、未確認理由保持を実装・単体検証。Worker、取消、stale、preview、適用、Undo、実時間性能は未実装 |
...
3. 正本のSchema・意味検証から検証済みProjectへ変換し、純粋探索を専用module Workerで実行するprotocol/client、取消、stale結果破棄を実装する。
...
| コンテナ・配置の自動提案 | [ADR 0004](../decisions/0004-optimization-objective.md)、[合成受入AP-01〜08](../acceptance.md#automatic-proposal-synthetic-cases) | 純粋な一候補DFS、目的順位、向き・候補点列挙、attempt予算、全積荷一度・正本validator再確認、cutoff集約、未確認理由保持を実装。利用者向け経路は未実装 | 専用31件・全単体659件、型・lint・build、全ブラウザ51件、AP-01〜05の数値fixture、1,000 bounds/2,048点、DFS backtrack、10,000/1,000,000境界、入力非変異、独立レビュー合格。Worker・UI・実時間AP-08は未実施 |
