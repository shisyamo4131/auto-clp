...
1. [案内付き人間評価](../evidence/phase1-human-ui-trial-4e6c680.md)で必要性が確認されたUI改善のうち、次に荷室外の非永続仮置き場とscene近傍の座標・履歴操作を、既存commandと安全境界を保って設計・実装・検証する。明示的な `＋` / `－`、荷室基準の視点復元、選択積荷近傍の許可向き90度回転、時間的に識別できる通知、保存Navigation Drawerは実装・自動回帰済みである。
