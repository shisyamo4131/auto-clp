...
仕様版 `0.18.1` と案件スキーマ版 `0.1.0` は別に管理する。仕様の文言変更だけでは案件スキーマ版を上げず、保存データの意味または形が変わる場合にだけスキーマ版を更新する。
...
- 初回位置は積荷の先頭許可向き、Z=0 mm、案件の積荷順、100 mm間隔、各積荷の許可向き全体から得る最大X/Y footprintから負X側の決定的な非重複gridとして派生する。現在向きではなく許可向き全体をセル寸法に使うため、一つの積荷のsession回転でoverrideを持たない他の未配置積荷も再配置されない。
...
8. 読込開始後に現在Project参照とProject/scene入力generationが変わっていない場合だけ、新しい案件状態へ一括置換する。
...
- `hasFullGeometricSupport(target, candidates)` — 旧和集合100%被覆の低レベル回帰用helperとして実装を保持するが、仕様0.18.1の現行支持区分には使用しない。
